...
export async function searchItems(searchTerm:String): Promise<Items[]> {
    ...
    const url = 'https://s1.ariba.com/Buyer/soap/wal-mart-supp-APAC-T/WSCatalogItemSearch';
    const sampleHeaders = {
        'Accept-Encoding': 'gzip,deflate',
        'Content-Type': 'text/xml;charset=UTF-8',
        'SOAPAction': "/Process Definition",
        'Authorization': 'Basic c2FwdXNlcjpXYWxtYXJ0QDEyMzQ1Ng==',
    ...
    const { response } = await soapRequest({ url: url, headers: sampleHeaders, xml: searchxml, timeout: 10000 }); // Optional timeout parameter(milliseconds)
...
export async function createCart(cart:Cart): Promise<String> {
    ...
    const url = 'https://s1.ariba.com/Buyer/soap/wal-mart-supp-APAC-T/RequisitionImportPull';
    const sampleHeaders = {
        'Accept-Encoding': 'gzip,deflate',
        'Content-Type': 'text/xml;charset=UTF-8',
        'SOAPAction': "/Process Definition",
    ...
    let reqNumber;
    const { response } = await soapRequest({ url: url, headers: sampleHeaders, xml: inputxml, timeout: 10000 }); // Optional timeout parameter(milliseconds)
    const { headers, body, statusCode } = response;
    if (statusCode == 200){
        var result = xmlParser.toJson(body, {
            object: true,
